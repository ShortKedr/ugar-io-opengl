...
Game::Game() {
    ...
    if (isStarted) {
        Vector2 playerPosition(std::rand() % (int)(GAME_FIELD_WIDTH / 2.0f) + GAME_FIELD_WIDTH / 2.0f,
                               std::rand() % (int)(GAME_FIELD_HEIGHT / 2.0f) + GAME_FIELD_HEIGHT / 2.0f);
        ugars[0] = std::make_unique<UgarPlayer>(playerPosition, 5.0f);
    }
    for (int i = 1; i < UGAR_AMOUNT; i++) {
        Vector2 aiPosition(std::rand() % (int)(GAME_FIELD_WIDTH / 2.0f) + GAME_FIELD_WIDTH / 2.0f,
                           std::rand() % (int)(GAME_FIELD_HEIGHT / 2.0f) + GAME_FIELD_HEIGHT / 2.0f);
...
void Game::Update() {
    if (ugars[0] != nullptr && isStarted) {
        cam->observedPosition.x = cam->observedPosition.x + (ugars[0]->position.x - cam->observedPosition.x) *
            CAMERA_LERP;
        cam->observedPosition.y = cam->observedPosition.y + (ugars[0]->position.y - cam->observedPosition.y) *
            CAMERA_LERP;

        float requiredScale = 0.0f;
        if (ugars[0]->radius < 60.0f) requiredScale = (cam->viewportSize.y / NORMAL_SCALE_RESOLUTION);
        else
            requiredScale = (cam->viewportSize.y / NORMAL_SCALE_RESOLUTION) * (NORMAL_SCALE_RADIUS / (int)(ugars[0]->
                radius));
        cam->scale += (requiredScale - cam->scale) * SCALE_LERP;
    }
    else if (ugars[1] != nullptr) {
        cam->observedPosition.x = cam->observedPosition.x + (ugars[1]->position.x - cam->observedPosition.x) *
            CAMERA_LERP;
        cam->observedPosition.y = cam->observedPosition.y + (ugars[1]->position.y - cam->observedPosition.y) *
            CAMERA_LERP;

        float requiredScale = 0.0f;
        if (ugars[1]->radius < 60.0f) requiredScale = (cam->viewportSize.y / NORMAL_SCALE_RESOLUTION);
        else
            requiredScale = (cam->viewportSize.y / NORMAL_SCALE_RESOLUTION) * (NORMAL_SCALE_RADIUS / (int)(ugars[1]->
                radius));
        cam->scale += (requiredScale - cam->scale) * SCALE_LERP;
    }
    ...
    for (int i = 0; i < UGAR_AMOUNT; i++) {
        ...
        if (ugars[i]->position.x + ugars[i]->radius > GAME_FIELD_WIDTH)
            ugars[i]->position.x = GAME_FIELD_WIDTH - ugars[
                i]->radius;
        ...
        if (ugars[i]->position.y + ugars[i]->radius > GAME_FIELD_HEIGHT)
            ugars[i]->position.y = GAME_FIELD_HEIGHT -
                ugars[i]->radius;
...
void Game::ObjectControlUpdate() {
    ...
    for (int i = 1; i < UGAR_AMOUNT; i++) {
        if (ugars[i] == nullptr && count < 2) {
            ...
            ugars[i] = std::make_unique<UgarAI>(aiPosition, 5.0f);
            SyncUgarRef(i);
            static_cast<UgarAI*>(ugars[i].get())->AssignAIData(foodRefs.data(), FOOD_AMOUNT, ugarRefs.data(),
                                                               UGAR_AMOUNT);
            count++;
        }
    }
...
void Game::Draw() {
    ...
    if (!isStarted) {
        ...
        const float centerX = static_cast<float>(GameWindow::GetWindowWidth()) / 2.0f;
        GLBasics::DrawStartText(centerX, 100, &textColor);
        GLBasics::DrawStartText(centerX, 102, &textColor, 0.25f, 1.015f);
    }
}
